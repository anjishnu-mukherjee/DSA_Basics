...
int dll_delete_by_value(dnode** hptr, dnode** tptr,int val);
void dll_reverse(dnode** hptr, dnode** tptr);
    ...
    while (1) 
    {
        ...
        switch (choice) 
        {
            ...
            case 7: 
                ...
                scanf("%d", &val);
                result = dll_delete_by_value(&head, &tail, val);
                if (result != INT_MIN) 
                    printf("Deleted value: %d\n", result);
                dll_print(&head);
                break;
            case 8: 
                dll_reverse(&head, &tail);
